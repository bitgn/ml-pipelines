...
def project_created(t: test.Env):
    id = t.next_id()
...
def set_update_timestamp(t: test.Env, dm: evt.DatasetMetadata, days=0, minutes = 0):
    time = t.time + timedelta(days=days, minutes=minutes)
    dm.update_timestamp = int(time.timestamp())
    dm.set_fields.append(evt.FIELD_UPDATE_TIMESTAMP)


def set_sample(t: test.Env, dm: evt.DatasetMetadata, text: str, type=evt.SAMPLE_KIND.TEXT):
    dm.sample_body = text.encode()
    dm.sample_kind = type
    dm.set_fields.append(evt.FIELD_SAMPLE)

def set_description(t: test.Env, dm: evt.DatasetMetadata, text: str):
    dm.description = text
    dm.set_fields.append(evt.FIELD_DESCRIPTION)

def set_data_format(t: test.Env, dm:evt.DatasetMetadata, text: str):
    dm.data_format = text
    dm.set_fields.append(evt.FIELD_DATA_FORMAT)



def dataset_created(t: test.Env, e: evt.ProjectCreated):
    id = t.next_id()
    a = random.choice(adj)
    n = random.choice(nouns)
    ...
    meta.raw_bytes = random.randint(100, 2 ** 30)
    meta.zip_bytes = random.randint(int(meta.raw_bytes / 2), int(meta.raw_bytes))
    meta.file_count = random.randint(1, meta.record_count)


    set_update_timestamp(t, meta, minutes=-random.randint(0, 7 * 24 * 60))

    meta.set_fields.extend(
        [
    ...
    if kind == 1:
        ...
        set_sample(t, meta, text, evt.SAMPLE_KIND.JSON)
